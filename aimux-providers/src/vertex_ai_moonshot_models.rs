//! Moonshot AI models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_moonshot_models";
...
/// supplied via [`VertexAiMoonshotModelsConfig::with_base_url`]; prefer
/// [`VertexAiMoonshotModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the Moonshot AI Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiMoonshotModelsConfig(OpenAIConfig);

impl VertexAiMoonshotModelsConfig {
...
/// Moonshot AI Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
...
impl Provider for VertexAiMoonshotModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
