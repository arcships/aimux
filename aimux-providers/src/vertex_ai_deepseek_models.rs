//! DeepSeek models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
/// supplied via [`VertexAiDeepseekModelsConfig::with_base_url`]; prefer
/// [`VertexAiDeepseekModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the DeepSeek Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiDeepseekModelsConfig(OpenAIConfig);

impl VertexAiDeepseekModelsConfig {
...
impl Provider for VertexAiDeepseekModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
