//! Z.AI models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_zai_models";
...
/// supplied via [`VertexAiZaiModelsConfig::with_base_url`]; prefer
/// [`VertexAiZaiModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the Z.AI Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiZaiModelsConfig(OpenAIConfig);

impl VertexAiZaiModelsConfig {
...
/// Z.AI Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
...
impl Provider for VertexAiZaiModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
