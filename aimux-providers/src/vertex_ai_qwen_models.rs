//! Qwen models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_qwen_models";
...
/// supplied via [`VertexAiQwenModelsConfig::with_base_url`]; prefer
/// [`VertexAiQwenModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the Qwen Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiQwenModelsConfig(OpenAIConfig);

impl VertexAiQwenModelsConfig {
...
/// Qwen Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
...
impl Provider for VertexAiQwenModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
