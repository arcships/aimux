//! Anthropic Claude models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_anthropic_models";
...
/// supplied via [`VertexAiAnthropicModelsConfig::with_base_url`]; prefer
/// [`VertexAiAnthropicModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the Anthropic Claude Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiAnthropicModelsConfig(OpenAIConfig);

impl VertexAiAnthropicModelsConfig {
...
/// Anthropic Claude Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
...
impl Provider for VertexAiAnthropicModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
