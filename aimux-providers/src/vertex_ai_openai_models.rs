//! OpenAI models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_openai_models";
...
/// supplied via [`VertexAiOpenaiModelsConfig::with_base_url`]; prefer
/// [`VertexAiOpenaiModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the OpenAI Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiOpenaiModelsConfig(OpenAIConfig);

impl VertexAiOpenaiModelsConfig {
...
/// OpenAI Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
...
impl Provider for VertexAiOpenaiModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
