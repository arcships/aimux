...
/// supplied via [`VertexAiMistralModelsConfig::with_base_url`]; prefer
/// [`VertexAiMistralModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the Mistral Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiMistralModelsConfig(OpenAIConfig);

impl VertexAiMistralModelsConfig {
...
impl Provider for VertexAiMistralModelsProvider {
    fn name(&self) -> &str {
        PROVIDER_NAME
    }
