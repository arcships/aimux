//! Kimi provider — a thin OpenAI-compatible wrapper.
//!
//! Exposes an OpenAI-compatible Chat Completions API at
//! `https://api.moonshot.ai/v1`. Provider-specific details are the base URL
...
/// Configuration for the Kimi provider (wraps [`OpenAIConfig`]).
pub struct KimiConfig(OpenAIConfig);
...
impl KimiConfig {
    ...
    pub fn from_env() -> Result<Self, AiMuxError> {
        let key = load_api_key(None, ENV_VAR, "Kimi")?;
        Ok(Self::new(key))
    }
...
/// Kimi provider — creates [`OpenAIModel`] instances.
...
impl KimiProvider {
    ...
    /// Create a model instance (e.g. `thinking`).
