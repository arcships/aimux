//! Doubao provider — a thin OpenAI-compatible wrapper.
//!
//! Exposes an OpenAI-compatible Chat Completions API at
//! `https://ark.cn-beijing.volces.com/api/v3`. Provider-specific details are the base URL
...
/// Configuration for the Doubao provider (wraps [`OpenAIConfig`]).
pub struct DoubaoConfig(OpenAIConfig);
...
impl DoubaoConfig {
    ...
    pub fn from_env() -> Result<Self, AiMuxError> {
        let key = load_api_key(None, ENV_VAR, "Doubao")?;
        Ok(Self::new(key))
    }
...
/// Doubao provider — creates [`OpenAIModel`] instances.
pub struct DoubaoProvider(OpenAIProvider);
...
impl DoubaoProvider {
    ...
    /// Create a model instance (e.g. `deepseek-v3-2-251201`).
