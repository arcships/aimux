//! Mimo provider — a thin OpenAI-compatible wrapper.
//!
//! Exposes an OpenAI-compatible Chat Completions API at
//! `https://api.xiaomimimo.com/v1`. Provider-specific details are the base URL
...
const PROVIDER_NAME: &str = "mimo";

/// Configuration for the Mimo provider (wraps [`OpenAIConfig`]).
pub struct MimoConfig(OpenAIConfig);
...
impl MimoConfig {
    ...
    pub fn from_env() -> Result<Self, AiMuxError> {
        let key = load_api_key(None, ENV_VAR, "Mimo")?;
        Ok(Self::new(key))
    }
...
/// Mimo provider — creates [`OpenAIModel`] instances.
pub struct MimoProvider(OpenAIProvider);
...
impl MimoProvider {
    ...
    /// Create a model instance (e.g. `mimo-v2.5-pro`).
