//! MiniMax models on Vertex AI MaaS — a thin OpenAI-compatible wrapper.
//!
...
const PROVIDER_NAME: &str = "vertex_ai_minimax_models";
...
/// supplied via [`VertexAiMinimaxModelsConfig::with_base_url`]; prefer
/// [`VertexAiMinimaxModelsConfig::from_env`] or set the project explicitly for real usage.
...
/// Configuration for the MiniMax Vertex AI MaaS provider (wraps [`OpenAIConfig`]).
pub struct VertexAiMinimaxModelsConfig(OpenAIConfig);

impl VertexAiMinimaxModelsConfig {
...
/// MiniMax Vertex AI MaaS provider — creates [`OpenAIModel`] instances pointed
