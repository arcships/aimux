...
async fn finding_10_bedrock_stream_reasoning_signature_emitted_as_metadata_delta() {
    ...
    // The signature arrives as a zero-length delta carrying provider metadata —
    // it used to be dropped entirely.
    let sig_delta = parts
        .iter()
        .find_map(|p| match p {
            StreamPart::ReasoningDelta {
                ...
                provider_metadata: Some(m),
                ..
            } if delta.is_empty() => Some(m),
            _ => None,
        })
        .expect("a zero-length ReasoningDelta must carry the signature");
    assert_eq!(sig_delta["amazonBedrock"]["signature"], json!(signature));
    assert_eq!(sig_delta["bedrock"]["signature"], json!(signature));
}
