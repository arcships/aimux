...
/// The `signature` values carried on reasoning deltas, in emission order.
...
async fn bedrock_stream_reasoning_and_text() {
    ...
    // The signature arrives on a trailing zero-length reasoning delta, mirroring
    // the TS stream, and is carried under both provider keys.
    assert_eq!(
        reasoning_signatures(&parts),
        vec![STREAM_SIGNATURE],
