...
5. **集中埋点**：在 HTTP 咽喉点统一埋点，而非逐个 provider 各打各的。
...
所有 registry provider（数量随 RFC-0017 注册表演进，250+）汇流到 `aimux-provider-utils/src/http.rs` 的三个函数：
...
3. ~~**是否在本 RFC 一并加 `#[instrument]` 到各 provider 的 `do_generate`**~~：**已解决**——阶段 1/2 均不做（registry provider 数量大、改造成本高），http 层 + core 顶层已覆盖核心信息。
