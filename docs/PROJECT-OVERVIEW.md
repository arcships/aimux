...
| AI providers | 172 (11 native + 145 OpenAI-compatible + 15 voice/image/video) |
...
|-------|------|-------------|
| `LanguageModel` | Text generation + streaming + tool calling | OpenAI / Anthropic / Google / DeepSeek / 172 providers |
...
### Categorization of 172 providers

| Type | Count | Representatives |
|------|:---:|------|
| Native protocol | 11 | OpenAI, Anthropic, Google, Bedrock, Vertex, Azure, Cohere, Mistral, xAI, DeepSeek |
| OpenAI-compatible | 145 | Groq, Fireworks, Together, Perplexity, Ollama, OpenRouter, Alibaba Tongyi, Zhipu, Baidu, Tencent, iFlytek, Moonshot AI, SiliconFlow… |
...
│   └── 145 OpenAI compatible #   thin wrapper, shared request path
...
aimux does not compete with LangChain; instead it serves as the layer beneath LangChain — LangChain handles the agent loop / RAG / chain, while aimux handles unified access to 172 providers. Running the access layer in Rust delivers performance and memory behavior far exceeding a JS-implemented access layer.
...
### 3. The way to unify 172 providers
...
The 11 native protocols have independent models + convert (handling differences such as Anthropic message format / Google generateContent / Bedrock SigV4), while the 145 OpenAI-compatible providers only need a thin wrapper + profile.
...
- [x] 172 providers integrated (11 native + 145 compatible + 15 voice/image/video)
...
| [rfc/0004-provider-inventory.md](../rfc/0004-provider-inventory.md) | 172 provider inventory |
