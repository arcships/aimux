...
| DeepSeek | compat(deepseek profile) | 自动,**磁盘全局共享缓存**(跨节点一致,集群路由影响最小——2026-08 实测表现最好) | 64 token(整单元) | **单元级完全匹配**(缓存前缀单元:请求边界/公共前缀检测/固定间隔持久化,非逐 token 前缀);前缀须从 token 0 | 闲置数小时-数天 | prompt_cache_hit_tokens→read(扁平字段) | prompt=hit+miss(官方);cached%64==0;命中 ≤ 客户端 LCP | 模型版本升级全失效;user_id 隔离;第二轮 0 命中合法;单元未持久化则公共前缀不命中(官方 Example 2) | A | 扁平字段名勿用 prompt_tokens_details 解析;多轮看公共前缀落盘后第 3 轮命中 |
