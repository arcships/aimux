...
| Model catalogue | [data/models/](../../aimux-providers/data/models/) | — (data files) | per-provider context window / pricing / modality data, generated by `scripts/gen_models.py` |
...
The complete list — 251 registry-backed (name / display / tier / env var /
base URL) and 78 typed-factory providers grouped by category — is in
...
Two more generators hang off the same registry, both with a `--check` mode
that CI runs: `scripts/gen_providers_doc.py --check` fails when
`providers.md` is stale, and `scripts/gen_models.py` writes the model
catalogue under `aimux-providers/data/models/` from models.dev and anya2a
(context windows, pricing, modalities; corrections go in
`data/models.overrides.json`, never in the generated files). Adding a
provider: [contributing/adding-a-provider.md](../contributing/adding-a-provider.md).
